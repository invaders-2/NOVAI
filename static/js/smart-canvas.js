...
function smartMediaPreviewUrl(itemOrUrl, size=512){ return window.NovaMedia ? NovaMedia.mediaPreviewUrlFromItem(itemOrUrl, size) : _localSmartMediaPreviewUrl(itemOrUrl, size); }
function _localSmartMediaPreviewUrl(itemOrUrl, size=512){
...
function smartPreviewImgHtml(itemOrUrl, size=512, attrs=''){ return window.NovaMedia ? NovaMedia.previewImgHtmlFromItem(itemOrUrl, size, attrs) : _localSmartPreviewImgHtml(itemOrUrl, size, attrs); }
function _localSmartPreviewImgHtml(itemOrUrl, size=512, attrs=''){
...
function loadSmartOriginalImageDimensions(url){ return window.NovaMedia ? NovaMedia.loadImageDimensions(url) : _localLoadSmartImageDimensions(url); }
function _localLoadSmartImageDimensions(url){
...
function _localSmartVideoPreviewHtml(itemOrUrl, size=512, attrs=''){
    const original = smartOriginalMediaUrl(itemOrUrl);
    const preview = smartMediaPreviewUrl(itemOrUrl, size);
...
function smartVideoFallbackHtml(url, attrs=''){ return window.NovaMedia ? NovaMedia.videoFallbackHtmlFromItem(url, attrs) : _localSmartVideoFallbackHtml(url, attrs); }
...
function startCanvasMetaPoll(){
    ...
    function poll(){
        canvasMetaPollTimer = setTimeout(async () => {
            if(!canvasId || !canvas) { poll(); return; }
            if(canvasSyncInFlight || dragState || selectionState) { poll(); return; }
            try {
...
function formatRunDuration(ms){
    if(window.NovaUtils) return NovaUtils.formatRunDuration(ms);
...
function renderPanoramaFrame(){
    /* —— document.hidden 检查：页面不可见时暂停 rAF 循环，避免浪费 CPU/GPU —— */
    if(document.hidden){
        panoramaState.pausedByHidden = true;
        return;
    }
...
function stopPanoramaLoop(){
    ...
    panoramaState.pausedByHidden = false;
}
/* 页面可见性变化时自动恢复全景渲染循环 */
document.addEventListener('visibilitychange', () => {
    if(!document.hidden && panoramaState.pausedByHidden && panoramaState.enabled){
        panoramaState.pausedByHidden = false;
        panoramaState.animationId = requestAnimationFrame(renderPanoramaFrame);
    }
});
...
/* ══════════════════════════════════════════════════════════════
   V2.5: 画布内功能抽屉 —— 输出 / 批量矩阵 / 视频分镜
   （复用后端 API，与独立页面 outputs.html / matrix.html / storyboard.html 功能等价）
   ══════════════════════════════════════════════════════════════ */
function canvasApi(path, opts){
...
function canvasModalOpen(title, html){
    document.getElementById('canvasModalTitle').textContent = title;
    document.getElementById('canvasModalBody').innerHTML = html;
    document.getElementById('canvasModalMask').classList.add('show');
    if(window.lucide) lucide.createIcons();
}
function canvasModalClose(){
    document.getElementById('canvasModalMask').classList.remove('show');
}
function canvasFmtSize(b){
    if(!b) return '';
    if(b > 1048576) return (b/1048576).toFixed(1) + 'MB';
    if(b > 1024) return Math.round(b/1024) + 'KB';
    return b + 'B';
}
function canvasFmtDate(ms){
    if(!ms) return '';
    const d = new Date(ms);
    return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0');
}
function canvasFmtTime(ts){
    if(!ts) return '-';
...
/* ── 抽屉开关 ── */
const CANVAS_PANELS = ['outputs','matrix','storyboard'];
const CanvasPanelState = {
    outputs:    { kind:'', fav:false, q:'', items:[], total:0, offset:0, limit:60 },
    matrix:     { view:'list', matrixId:null, matrix:null, timer:null },
    storyboard: { storyboards:[], sb:null, sceneId:null, timer:null, pickTab:'outputs', pickItems:[] }
};
function openCanvasPanel(name){
    CANVAS_PANELS.forEach(n => {
        const p = document.getElementById(n + 'Panel');
        if(!p) return;
        const btn = document.getElementById(n + 'PanelBtn');
        if(n === name){
    ...
    if(name === 'outputs') loadOutputsPanel();
    if(name === 'matrix') loadMatricesPanel();
    if(name === 'storyboard') loadStoryboardsPanel();
    if(name !== 'matrix') stopMatrixPolling();
    if(name !== 'storyboard') stopStoryboardPolling();
}
function closeCanvasPanel(name){
    const p = document.getElementById(name + 'Panel');
    if(p) p.classList.remove('open');
    const btn = document.getElementById(name + 'PanelBtn');
    if(btn) btn.classList.remove('active');
    if(name === 'matrix') stopMatrixPolling();
    if(name === 'storyboard') stopStoryboardPolling();
}
...
/* ── 输出面板 ── */
async function loadOutputsPanel(){
    const st = CanvasPanelState.outputs;
    const grid = document.getElementById('outputsGrid');
    if(grid) grid.innerHTML = '<div class="canvas-drawer-empty">加载中…</div>';
    try{
        const q = new URLSearchParams({limit: st.limit, offset: st.offset});
        if(st.kind) q.set('kind', st.kind);
        if(st.q) q.set('q', st.q);
        if(st.fav) q.set('favorites', '1');
        const d = await canvasApi('/api/outputs?' + q.toString());
        st.items = d.items || [];
        st.total = d.total || 0;
        renderOutputsPanel();
    }catch(e){
        if(grid) grid.innerHTML = '<div class="canvas-drawer-empty">加载失败：' + escapeHtml(e.message) + '</div>';
    }
}
function renderOutputsPanel(){
    ...
    const stats = document.getElementById('outputsStats');
    if(!grid) return;
    if(stats) stats.textContent = '共 ' + st.total + ' 个结果' + (st.kind ? ' · ' + st.kind : '') + (st.fav ? ' · 收藏' : '');
    grid.innerHTML = '';
    if(!st.items.length){ if(empty) empty.style.display = 'block'; return; }
    if(empty) empty.style.display = 'none';
    st.items.forEach(it => {
        const thumb = it.kind === 'video'
            ? `<video src="${escapeAttr(it.url)}#t=0.5" muted preload="metadata" playsinline></video>`
            : `<img src="${escapeAttr(it.url)}" loading="lazy" onerror="this.style.display='none'">`;
        const card = document.createElement('div');
        card.className = 'outputs-card';
        card.innerHTML = `
            <div class="thumb">${thumb}<span class="outputs-badge">${escapeHtml(it.kind)}</span>
                <button class="outputs-fav ${it.favorite?'on':''}" onclick="event.stopPropagation();outputsToggleFav('${it.id}')">${it.favorite?'★':'☆'}</button></div>
            <div class="outputs-meta"><div class="outputs-name">${escapeHtml(it.name)}</div>
                <div class="outputs-info"><span>${canvasFmtSize(it.size)}</span><span>${canvasFmtDate(it.created_at)}</span></div></div>
            <div class="outputs-actions">
                <button onclick="event.stopPropagation();outputsPreview('${it.id}')">预览</button>
                <button onclick="event.stopPropagation();outputsDownload('${it.id}')">下载</button>
                <button onclick="event.stopPropagation();outputsAddToCanvas('${it.id}')">加入画布</button>
            </div>`;
        card.addEventListener('click', () => outputsPreview(it.id));
        grid.appendChild(card);
    });
}
function outputsSetKind(el){
    document.querySelectorAll('#outputsTabs .canvas-tab[data-kind]').forEach(t => t.classList.remove('active'));
    el.classList.add('active');
    const st = CanvasPanelState.outputs;
    st.kind = el.dataset.kind || '';
    st.fav = false;
    const favTab = document.querySelector('#outputsTabs .canvas-tab[data-fav]');
    if(favTab) favTab.classList.remove('active');
    st.offset = 0;
    loadOutputsPanel();
}
function outputsSetFav(el){
    const st = CanvasPanelState.outputs;
    st.fav = !st.fav;
    el.classList.toggle('active', st.fav);
    document.querySelectorAll('#outputsTabs .canvas-tab[data-kind]').forEach(t => t.classList.remove('active'));
    st.kind = '';
    st.offset = 0;
    loadOutputsPanel();
}
let outputsSearchTimer = null;
function outputsSearchInput(){
    clearTimeout(outputsSearchTimer);
    outputsSearchTimer = setTimeout(() => {
        const st = CanvasPanelState.outputs;
        st.q = (document.getElementById('outputsSearch').value || '').trim();
        st.offset = 0;
        loadOutputsPanel();
    }, 400);
}
function refreshOutputsPanel(){ loadOutputsPanel(); }
async function outputsToggleFav(id){
    try{
        const st = CanvasPanelState.outputs;
        const it = st.items.find(x => x.id === id);
        if(!it) return;
        if(it.favorite) await canvasApi('/api/outputs/' + encodeURIComponent(id) + '/favorite', {method:'DELETE'});
        else await canvasApi('/api/outputs/' + encodeURIComponent(id) + '/favorite', {method:'POST'});
        it.favorite = !it.favorite;
        renderOutputsPanel();
    }catch(e){ toast('操作失败：' + e.message); }
}
async function outputsPreview(id){
    const it = CanvasPanelState.outputs.items.find(x => x.id === id);
    if(!it) return;
    const body = it.kind === 'video'
        ? `<video src="${escapeAttr(it.url)}" controls autoplay playsinline></video><div class="m-name">${escapeHtml(it.name)}</div>`
        : `<img src="${escapeAttr(it.url)}"><div class="m-name">${escapeHtml(it.name)}</div>`;
    canvasModalOpen(it.name, body);
}
function outputsDownload(id){
    const it = CanvasPanelState.outputs.items.find(x => x.id === id);
    if(!it) return;
...
async function outputsAddToCanvas(id){
    try{
        const d = await canvasApi('/api/canvases');
        const canvases = (d.canvases || []).filter(c => !c.deleted_at);
        const rows = canvases.map(c => `
            <div class="pick-item-row" onclick="outputsDoAddToCanvas('${id}', '${c.id}')">
                <span class="t">${escapeHtml(c.title || '未命名')}</span>
                <span class="k">${c.kind === 'smart' ? '智能画布' : '普通画布'}${c.id === canvasId ? ' · 当前' : ''}</span>
            </div>`).join('');
        canvasModalOpen('选择目标画布',
            (canvasId ? `<button class="canvas-btn primary" style="width:100%;margin-bottom:8px" onclick="outputsDoAddToCanvas('${id}', '${canvasId}')">加入当前画布</button>` : '') +
            rows +
            '<div class="canvas-drawer-toolbar" style="margin-top:10px;justify-content:flex-end"><button class="canvas-btn" onclick="canvasModalClose()">取消</button></div>');
    }catch(e){ toast('加载画布列表失败：' + e.message); }
}
async function outputsDoAddToCanvas(outputId, targetCanvasId){
    try{
        await canvasApi('/api/outputs/' + encodeURIComponent(outputId) + '/add-to-canvas', {
            method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({canvas_id: targetCanvasId})
        });
        canvasModalClose();
        toast('已加入画布');
    }catch(e){ toast('加入画布失败：' + e.message); }
}

/* ── 批量矩阵面板 ── */
const MATRIX_STATUS_TEXT = { queued:'排队中', running:'执行中', success:'成功', failed:'失败', cancelled:'已取消' };
const MATRIX_STATUS_ORDER = ['total','queued','running','success','failed','cancelled'];
const MATRIX_STATUS_COLOR = { queued:'var(--info)', running:'var(--warning)', success:'var(--success)', failed:'var(--danger)', cancelled:'var(--text-3)' };
async function loadMatricesPanel(){
    const st = CanvasPanelState.matrix;
    if(st.view === 'detail'){ refreshMatrixDetail(); return; }
    try{
        const d = await canvasApi('/api/matrices');
        renderMatrixList(d.items || []);
    }catch(e){ toast('加载失败：' + e.message); }
}
function refreshMatricesPanel(){ loadMatricesPanel(); }
function renderMatrixList(items){
    const grid = document.getElementById('matrixListGrid');
    const empty = document.getElementById('matrixListEmpty');
    const legend = document.getElementById('matrixListLegend');
    if(!grid) return;
    grid.innerHTML = '';
    if(!items.length){ if(empty) empty.style.display = 'block'; if(legend) legend.innerHTML = ''; return; }
    if(empty) empty.style.display = 'none';
    const all = {queued:0, running:0, success:0, failed:0, cancelled:0, total:0};
    items.forEach(m => { const s = m.stats || {}; MATRIX_STATUS_ORDER.forEach(k => { if(k !== 'total') all[k] += (s[k]||0); }); all.total += (s.total||0); });
    if(legend) legend.innerHTML = MATRIX_STATUS_ORDER.filter(k=>k!=='total').map(k => `<span class="item"><span class="dot" style="background:${MATRIX_STATUS_COLOR[k]}"></span>${MATRIX_STATUS_TEXT[k]} ${all[k]}</span>`).join('') + `<span class="item">共 ${all.total} 个单元格</span>`;
    items.forEach(m => {
        const s = m.stats || {};
        ...
        card.innerHTML = `
            <button class="del" title="删除矩阵" onclick="event.stopPropagation();matrixDelete('${m.matrix_id}')">✕</button>
            <div class="matrix-name">${escapeHtml(m.name)}</div>
            <div class="matrix-dims">${dims.map(([k,v]) => `<b>${escapeHtml(k)}</b>×${escapeHtml((v||[]).length)}`).join(' · ')}</div>
            <div class="matrix-mini">
                <span class="matrix-pill ${(s.success||0)>0?'ok':''}">✓ ${s.success||0}</span>
                <span class="matrix-pill ${(s.failed||0)>0?'bad':''}">✗ ${s.failed||0}</span>
                <span class="matrix-pill">⏳ ${(s.running||0)+(s.queued||0)}</span>
                <span>${canvasFmtTime(m.created_at)}</span>
            </div>`;
        card.addEventListener('click', () => openMatrixDetail(m.matrix_id));
        grid.appendChild(card);
    });
}
async function openMatrixDetail(id){
    const st = CanvasPanelState.matrix;
    st.view = 'detail'; st.matrixId = id;
    document.getElementById('matrixViewList').style.display = 'none';
    document.getElementById('matrixViewCreate').style.display = 'none';
    document.getElementById('matrixViewDetail').style.display = 'flex';
    await refreshMatrixDetail();
    startMatrixPolling();
}
function matrixBackToList(){
    stopMatrixPolling();
    const st = CanvasPanelState.matrix;
    st.view = 'list'; st.matrix = null; st.matrixId = null;
    document.getElementById('matrixViewDetail').style.display = 'none';
    document.getElementById('matrixViewCreate').style.display = 'none';
    document.getElementById('matrixViewList').style.display = 'flex';
    loadMatricesPanel();
}
function startMatrixPolling(){
    stopMatrixPolling();
    CanvasPanelState.matrix.timer = setInterval(refreshMatrixDetail, 3000);
}
function stopMatrixPolling(){
    if(CanvasPanelState.matrix.timer){ clearInterval(CanvasPanelState.matrix.timer); CanvasPanelState.matrix.timer = null; }
}
async function refreshMatrixDetail(){
    const st = CanvasPanelState.matrix;
    if(st.view !== 'detail' || !st.matrixId) return;
    try{
        const m = await canvasApi('/api/matrices/' + encodeURIComponent(st.matrixId));
        st.matrix = m;
        renderMatrixDetail(m);
    }catch(e){ if(st.view === 'detail') toast('详情刷新失败：' + e.message); }
}
function renderMatrixDetail(m){
    document.getElementById('matrixDetailName').textContent = m.name;
    document.getElementById('matrixDetailSub').textContent = 'ID ' + m.matrix_id + ' · 创建于 ' + canvasFmtTime(m.created_at) + ' · 更新于 ' + canvasFmtTime(m.updated_at);
    document.getElementById('matrixDetailDims').innerHTML = Object.entries(m.dimensions || {}).map(([k,v]) => `<span class="matrix-dim-chip"><b>${escapeHtml(k)}</b>${escapeHtml((v||[]).join(' / '))}</span>`).join('');
    const s = m.stats || {};
    const total = Math.max(1, s.total || 0);
    document.getElementById('matrixDetailStats').innerHTML = MATRIX_STATUS_ORDER.filter(k=>k!=='total').map(k => {
        const n = s[k]||0;
        if(!n) return `<div class="matrix-seg s-${k} zero" title="${MATRIX_STATUS_TEXT[k]} 0">0</div>`;
        return `<div class="matrix-seg s-${k}" style="width:${Math.max(6, n/total*100).toFixed(2)}%" title="${MATRIX_STATUS_TEXT[k]} ${n}">${n}</div>`;
    }).join('');
    document.getElementById('matrixDetailLegend').innerHTML = MATRIX_STATUS_ORDER.filter(k=>k!=='total').map(k => `<span class="item"><span class="dot" style="background:${MATRIX_STATUS_COLOR[k]}"></span>${MATRIX_STATUS_TEXT[k]} ${s[k]||0}</span>`).join('') + `<span class="item">共 ${s.total||0} 个单元格</span>`;
    const cells = document.getElementById('matrixDetailCells');
    cells.innerHTML = '';
    (m.cells || []).forEach(c => {
        ...
        div.innerHTML = `
            <div class="strip" style="background:${MATRIX_STATUS_COLOR[st] || 'var(--info)'}"></div>
            <div class="combo">${Object.entries(c.combo||{}).map(([k,v]) => `<span>${escapeHtml(k)}</span>：${escapeHtml(v)}`).join('<br>')}</div>
            <span class="cstatus ${hasTask?'s-'+st:'norun'}">${hasTask ? (MATRIX_STATUS_TEXT[st]||st) : '未运行'}</span>
            <div class="ctask">${hasTask ? escapeHtml(c.task_id) : ''}</div>`;
        div.addEventListener('click', () => {
            if(!c.task_id){ toast('该单元格尚未运行（点击「运行」后生成任务）'); return; }
            openCanvasTaskModal(c.task_id);
        });
        cells.appendChild(div);
    });
}
async function matrixRun(){
    const m = CanvasPanelState.matrix.matrix; if(!m) return;
    if(!window.confirm(`运行「${m.name}」？将为 ${m.stats.total} 个单元格创建真实生成任务并进入执行队列（已结束的单元格会重新创建任务）。`)) return;
    try{
        const r = await canvasApi(`/api/matrices/${encodeURIComponent(m.matrix_id)}/run`, {method:'POST'});
        toast(r.message || `已创建 ${r.launched} 个任务`);
        await refreshMatrixDetail();
    }catch(e){ toast('运行失败：' + e.message); }
}
async function matrixCancel(){
    const m = CanvasPanelState.matrix.matrix; if(!m) return;
    if(!window.confirm(`取消「${m.name}」所有执行中/排队中的任务？`)) return;
    try{
        const r = await canvasApi(`/api/matrices/${encodeURIComponent(m.matrix_id)}/cancel`, {method:'POST'});
        toast(r.message || `已受理 ${r.cancelled} 个取消`);
        await refreshMatrixDetail();
    }catch(e){ toast('取消失败：' + e.message); }
}
async function matrixDelete(id){
    const mid = id || (CanvasPanelState.matrix.matrix && CanvasPanelState.matrix.matrix.matrix_id);
    if(!mid) return;
    if(!window.confirm('删除该矩阵记录？（已生成产物与任务历史不受影响）')) return;
    try{
        await canvasApi('/api/matrices/' + encodeURIComponent(mid), {method:'DELETE'});
        toast('已删除');
        if(CanvasPanelState.matrix.view === 'detail') matrixBackToList(); else loadMatricesPanel();
    }catch(e){ toast('删除失败：' + e.message); }
}
/* 新建矩阵 */
function openMatrixCreatePanel(){
    const st = CanvasPanelState.matrix;
    stopMatrixPolling();
    st.view = 'create';
    document.getElementById('matrixViewList').style.display = 'none';
    document.getElementById('matrixViewDetail').style.display = 'none';
    document.getElementById('matrixViewCreate').style.display = 'flex';
    document.getElementById('matrixCreateDims').innerHTML = '';
    matrixAddDimRow('角色', '厨师,模特,博主');
    matrixAddDimRow('产品', '咖啡机,耳机');
    matrixAddDimRow('场景', '工作室,户外');
    document.getElementById('matrixCreateName').value = '';
    document.getElementById('matrixCreateParams').value = JSON.stringify({prompt:'一张电商主图：{角色} 正在展示 {产品}，背景为 {场景}，高清商业摄影', provider_id:'comfly', model:'flux', size:'1024x1024', quality:'auto', n:1}, null, 2);
    matrixPreviewCount();
}
function matrixAddDimRow(name, vals){
    const box = document.getElementById('matrixCreateDims');
    const row = document.createElement('div');
    row.className = 'matrix-dim-row';
    row.innerHTML = `
        <input class="dname" placeholder="维度名（如 角色）" value="${escapeAttr(name||'')}">
        <input class="dvals" placeholder="值，逗号分隔（如 厨师,模特,博主）" value="${escapeAttr(vals||'')}">
        <button class="del" onclick="this.parentElement.remove();matrixPreviewCount()" title="删除维度">✕</button>`;
    row.querySelectorAll('input').forEach(i => i.addEventListener('input', matrixPreviewCount));
    box.appendChild(row);
    matrixPreviewCount();
}
function matrixCollectDims(){
    const dims = {};
    document.querySelectorAll('#matrixCreateDims .matrix-dim-row').forEach(row => {
...
function matrixPreviewCount(){
    const dims = matrixCollectDims();
    let n = 1; Object.values(dims).forEach(v => n *= v.length);
    const el = document.getElementById('matrixCreateCount');
    if(el) el.textContent = Object.keys(dims).length ? `将生成 ${n} 个单元格（${Object.entries(dims).map(([k,v]) => `${k}×${v.length}`).join(' × ')}）` : '请添加至少一个维度';
}
async function matrixCreate(){
    const name = document.getElementById('matrixCreateName').value.trim();
    const dims = matrixCollectDims();
    let params;
    try{ params = JSON.parse(document.getElementById('matrixCreateParams').value || '{}'); }
    catch(e){ toast('参数模板不是合法 JSON：' + e.message); return; }
    if(!name){ toast('请填写矩阵名称'); return; }
    if(!Object.keys(dims).length){ toast('请至少添加一个维度'); return; }
    if(!(params.prompt || '').trim()){ toast('参数模板缺少 prompt（提示词模板）'); return; }
    try{
        const m = await canvasApi('/api/matrices', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({name, dimensions: dims, base_params: params})});
        toast(`已创建「${m.name}」，共 ${m.stats.total} 个单元格`);
        openMatrixDetail(m.matrix_id);
    }catch(e){ toast('创建失败：' + e.message); }
}

/* ── 任务详情弹窗（矩阵/分镜共用）── */
const CANVAS_TASK_RAW_TEXT = { queued:'排队中', running:'执行中', provider_processing:'上游生成中', downloading:'下载中', saving:'保存中', succeeded:'成功', failed:'失败', cancelled:'已取消', cancel_requested:'取消中', retry:'重试中', jimeng_pending:'即梦排队中' };
let CanvasTaskModalId = null;
async function openCanvasTaskModal(taskId){
    CanvasTaskModalId = taskId;
    canvasModalOpen('任务详情', '<div class="task-line"><span class="k">加载中</span><span class="v">…</span></div>');
    try{
        const t = await canvasApi('/api/tasks/' + encodeURIComponent(taskId));
        renderCanvasTaskModal(t);
    }catch(e){
        document.getElementById('canvasModalBody').innerHTML = `<div class="task-line"><span class="k">错误</span><span class="v" style="color:var(--danger)">${escapeHtml(e.message)}</span></div>`;
    }
}
function renderCanvasTaskModal(t){
    ...
    if(result && typeof result === 'object'){
        ...
        urls.filter(Boolean).slice(0,8).forEach(u => imgs.push(`<img src="${escapeAttr(u)}" loading="lazy" onclick="window.open('${escapeAttr(u)}','_blank')" title="点击放大">`));
    }
    const params = t.parameters || t.payload || {};
    document.getElementById('canvasModalBody').innerHTML = `
        <div class="task-line"><span class="k">任务 ID</span><span class="v">${escapeHtml(t.id)}</span></div>
        <div class="task-line"><span class="k">状态</span><span class="v"><span class="task-status ${stCls}">${escapeHtml(CANVAS_TASK_RAW_TEXT[st] || st)}</span></span></div>
        <div class="task-line"><span class="k">类型</span><span class="v">${escapeHtml(t.kind || t.type || '-')}${t.matrix_id ? ' · Matrix: ' + escapeHtml(t.matrix_name || t.matrix_id) : ''}${t.storyboard_id ? ' · 分镜: ' + escapeHtml(t.storyboard_name || t.storyboard_id) + (t.scene_order != null ? ' / 场景 ' + (t.scene_order+1) : '') : ''}</span></div>
        ${t.combo && typeof t.combo === 'object' && Object.keys(t.combo).length ? `<div class="task-line"><span class="k">组合</span><span class="v">${Object.entries(t.combo).map(([k,v]) => `${escapeHtml(k)}=${escapeHtml(v)}`).join('，')}</span></div>` : ''}
        <div class="task-line"><span class="k">提供商</span><span class="v">${escapeHtml(t.provider_id || '-')} / ${escapeHtml(t.model || t.model_id || '-')}</span></div>
        <div class="task-line"><span class="k">创建时间</span><span class="v">${canvasFmtTime(t.created_at)}</span></div>
        <div class="task-line"><span class="k">参数</span><span class="v"><pre>${escapeHtml(JSON.stringify(params, null, 2).slice(0, 1200))}</pre></span></div>
        ${t.error ? `<div class="task-line"><span class="k">错误</span><span class="v" style="color:var(--danger)">${escapeHtml(t.error)}</span></div>` : ''}
        ${imgs.length ? `<div class="result-imgs">${imgs.join('')}</div>` : ''}
        <div class="transitions">${(t.transitions||[]).map(tr => `<div class="transition"><span>${escapeHtml(tr.from || '—')}</span><span class="arrow">→</span><span>${escapeHtml(tr.to)}</span><span>${canvasFmtTime(tr.at)}</span>${tr.error ? `<span class="err" title="${escapeAttr(tr.error)}">${escapeHtml(String(tr.error).slice(0, 80))}</span>` : ''}</div>`).join('')}</div>
        <div class="canvas-drawer-toolbar" style="margin-top:12px;justify-content:flex-end">
            <button class="canvas-btn" id="canvasTaskRetryBtn" style="${['failed','cancelled','jimeng_pending'].includes(st) ? '' : 'display:none'}" onclick="canvasTaskRetry()">重试</button>
            <button class="canvas-btn" id="canvasTaskCancelBtn" style="${['succeeded','failed','cancelled','cancel_requested'].includes(st) ? 'display:none' : ''}" onclick="canvasTaskCancel()">取消任务</button>
            <button class="canvas-btn" onclick="canvasModalClose()">关闭</button>
        </div>`;
}
async function canvasTaskRetry(){
    if(!CanvasTaskModalId) return;
    try{ await canvasApi(`/api/tasks/${encodeURIComponent(CanvasTaskModalId)}/retry`, {method:'POST'}); toast('已重新排队'); openCanvasTaskModal(CanvasTaskModalId); }
    catch(e){ toast('重试失败：' + e.message); }
}
async function canvasTaskCancel(){
    if(!CanvasTaskModalId) return;
    try{ await canvasApi(`/api/tasks/${encodeURIComponent(CanvasTaskModalId)}/cancel`, {method:'POST'}); toast('已受理取消'); openCanvasTaskModal(CanvasTaskModalId); }
    catch(e){ toast('取消失败：' + e.message); }
}

/* ── 分镜面板 ── */
const SB_STATUS_TEXT = { queued:'排队中', running:'执行中', success:'成功', failed:'失败', cancelled:'已取消' };
async function loadStoryboardsPanel(){
    const st = CanvasPanelState.storyboard;
    try{
        const d = await canvasApi('/api/storyboards');
        st.storyboards = d.items || [];
        renderStoryboardSelect();
        if(!st.sb && st.storyboards.length){
            let saved = null;
            try{ saved = localStorage.getItem('studio_storyboard_id'); }catch(_){}
            const target = saved && st.storyboards.some(s => s.storyboard_id === saved) ? saved : st.storyboards[0].storyboard_id;
            await openStoryboardPanelItem(target);
        }
    }catch(e){ toast('加载失败：' + e.message); }
}
function refreshStoryboardsPanel(){ loadStoryboardsPanel(); }
function renderStoryboardSelect(){
    const st = CanvasPanelState.storyboard;
    const sel = document.getElementById('storyboardSelect');
    if(!sel) return;
    sel.innerHTML = '<option value="">选择分镜…</option>' + st.storyboards.map(s => `<option value="${escapeAttr(s.storyboard_id)}" ${st.sb && st.sb.storyboard_id === s.storyboard_id ? 'selected' : ''}>${escapeHtml(s.name)}（${s.scene_count||0}）</option>`).join('');
    const has = !!st.sb;
    document.getElementById('storyboardRenderAllBtn').disabled = !has;
    document.getElementById('storyboardDeleteBtn').disabled = !has;
    document.getElementById('storyboardAddSceneBtn').style.display = has ? '' : 'none';
}
async function storyboardSwitch(id){
    const st = CanvasPanelState.storyboard;
    if(!id){
        st.sb = null; st.sceneId = null;
        stopStoryboardPolling();
        renderStoryboardScenes(); renderStoryboardEditor(); renderStoryboardSelect();
        return;
    }
    await openStoryboardPanelItem(id);
}
async function openStoryboardPanelItem(id){
    const st = CanvasPanelState.storyboard;
    try{
        const sb = await canvasApi('/api/storyboards/' + encodeURIComponent(id));
        st.sb = sb;
        st.sceneId = (sb.scenes && sb.scenes[0]) ? sb.scenes[0].scene_id : null;
    ...
    }catch(e){ toast('打开分镜失败：' + e.message); }
}
function sortedStoryboardScenes(){
    const st = CanvasPanelState.storyboard;
    return [...(st.sb?.scenes || [])].sort((a,b) => (a.order||0) - (b.order||0) || String(a.scene_id).localeCompare(String(b.scene_id)));
}
function renderStoryboardScenes(){
    const st = CanvasPanelState.storyboard;
    const list = document.getElementById('storyboardList');
    if(!list) return;
    list.innerHTML = '';
    if(!st.sb) return;
    const scenes = sortedStoryboardScenes();
    scenes.forEach((s, idx) => {
        const status = s.status || 'queued';
        const hasTask = !!s.task_id;
        const card = document.createElement('div');
        card.className = 'storyboard-scene' + (st.sceneId === s.scene_id ? ' active' : '');
        card.draggable = true;
        card.dataset.sceneId = s.scene_id;
        card.innerHTML = `
            <div class="storyboard-scene-top">
                <span class="storyboard-scene-no">${idx+1}</span>
                <span class="storyboard-sdot ${hasTask ? 's-' + status : 'norun'}" title="${hasTask ? (SB_STATUS_TEXT[status]||status) : '未渲染'}"></span>
                ${s.duration ? `<span class="storyboard-scene-dur">${s.duration}s</span>` : ''}
                ${s.transition ? `<span class="storyboard-scene-dur">转场:${escapeHtml(s.transition)}</span>` : ''}
                <span class="storyboard-scene-tools">
                    <button title="上移" onclick="event.stopPropagation();storyboardMoveScene('${s.scene_id}',-1)">↑</button>
                    <button title="下移" onclick="event.stopPropagation();storyboardMoveScene('${s.scene_id}',1)">↓</button>
                    <button class="del" title="删除场景" onclick="event.stopPropagation();storyboardDeleteScene('${s.scene_id}')">✕</button>
                </span>
            </div>
            <div class="storyboard-scene-main">
                <div class="storyboard-scene-thumb">${s.visual ? `<img src="${escapeAttr(s.visual)}" loading="lazy" onerror="this.parentElement.innerHTML='<span>?</span>'">` : '<span>🎬</span>'}</div>
                <div class="storyboard-scene-meta">
                    <div class="storyboard-scene-prompt ${s.prompt ? '' : 'empty'}">${s.prompt ? escapeHtml(s.prompt) : '（空提示词）'}</div>
                    ${(s.camera || s.motion) ? `<div class="storyboard-scene-meta2">${escapeHtml([s.camera, s.motion].filter(Boolean).join(' / '))}</div>` : ''}
                </div>
            </div>`;
        card.addEventListener('click', () => { st.sceneId = s.scene_id; renderStoryboardScenes(); renderStoryboardEditor(); });
        card.addEventListener('dragstart', e => { e.dataTransfer.setData('text/plain', s.scene_id); card.classList.add('dragging'); });
        card.addEventListener('dragend', () => { card.classList.remove('dragging'); document.querySelectorAll('.storyboard-scene.drag-over').forEach(c => c.classList.remove('drag-over')); });
        ...
        card.addEventListener('drop', e => {
            ...
            storyboardReorderScene(fromId, s.scene_id);
        });
...
function renderStoryboardEditor(){
    const st = CanvasPanelState.storyboard;
    const sb = st.sb;
    const editor = document.getElementById('storyboardEditor');
    if(!editor) return;
    const scene = (sb?.scenes || []).find(s => s.scene_id === st.sceneId) || null;
    editor.style.display = sb ? 'block' : 'none';
    if(!sb) return;
    document.getElementById('storyboardName').value = sb.name || '';
    document.getElementById('storyboardCfgProvider').value = sb.video_config?.provider_id || 'comfly';
    document.getElementById('storyboardCfgModel').value = sb.video_config?.model || 'veo3-fast';
    document.getElementById('storyboardCfgDuration').value = sb.video_config?.duration || 5;
    document.getElementById('storyboardCfgAspect').value = sb.video_config?.aspect_ratio || '16:9';
    if(!scene){
        ['storyboardVisual','storyboardPrompt','storyboardCamera','storyboardMotion','storyboardAudio'].forEach(id => { const el = document.getElementById(id); if(el) el.value = ''; });
        document.getElementById('storyboardDuration').value = '';
        document.getElementById('storyboardTransition').value = '';
        document.getElementById('storyboardVisualPreview').innerHTML = '<span>🎬</span>';
        setStoryboardStatus('norun', '未渲染', '');
        return;
    }
    document.getElementById('storyboardVisual').value = scene.visual || '';
    document.getElementById('storyboardPrompt').value = scene.prompt || '';
    document.getElementById('storyboardCamera').value = scene.camera || '';
    document.getElementById('storyboardMotion').value = scene.motion || '';
    document.getElementById('storyboardDuration').value = scene.duration || '';
    document.getElementById('storyboardTransition').value = scene.transition || '';
    document.getElementById('storyboardAudio').value = scene.audio || '';
    renderStoryboardVisualPreview(scene.visual || '');
    const status = scene.status || 'queued';
    setStoryboardStatus(scene.task_id ? 's-' + status : 'norun', scene.task_id ? (SB_STATUS_TEXT[status] || status) : '未渲染', scene.task_id || '');
}
function renderStoryboardVisualPreview(url){
    const box = document.getElementById('storyboardVisualPreview');
    if(!box) return;
    if(!url){ box.innerHTML = '<span>🎬</span>'; return; }
    box.innerHTML = `<img src="${escapeAttr(url)}" onerror="this.parentElement.innerHTML='<span>?</span>'"><button class="clear" title="清除" onclick="storyboardClearVisual()">✕</button>`;
}
function setStoryboardStatus(cls, text, taskId){
    const chip = document.getElementById('storyboardStatus');
    if(chip){ chip.className = 'storyboard-status ' + cls; chip.textContent = text; }
    const tid = document.getElementById('storyboardTaskId');
    if(tid) tid.innerHTML = taskId ? `任务：<a onclick="openCanvasTaskModal('${escapeAttr(taskId)}')">${escapeHtml(taskId)}</a>` : '';
}
function storyboardClearVisual(){
    document.getElementById('storyboardVisual').value = '';
    renderStoryboardVisualPreview('');
}
async function storyboardAddScene(){
    const st = CanvasPanelState.storyboard;
    if(!st.sb) return;
    const scenes = sortedStoryboardScenes();
    scenes.push({scene_id:'', order: scenes.length, visual:'', prompt:'', camera:'', motion:'', duration:0, transition:'', audio:''});
    try{
        const sb = await storyboardSavePayload(scenes, st.sb.name);
        st.sb = sb;
        st.sceneId = (sb.scenes[sb.scenes.length - 1] || {}).scene_id;
        renderStoryboardScenes(); renderStoryboardEditor();
        toast('已新增场景，编辑后点「保存」');
    }catch(e){ toast('新增场景失败：' + e.message); }
}
async function storyboardMoveScene(sceneId, dir){
    const st = CanvasPanelState.storyboard;
    const scenes = sortedStoryboardScenes();
    ...
    try{
        const sb = await storyboardSavePayload(scenes, st.sb.name);
        st.sb = sb; st.sceneId = sceneId;
        renderStoryboardScenes(); renderStoryboardEditor();
    }catch(e){ toast('排序保存失败：' + e.message); }
}
function storyboardReorderScene(fromId, toId){
    const st = CanvasPanelState.storyboard;
    const scenes = sortedStoryboardScenes();
    ...
    storyboardSavePayload(scenes, st.sb.name).then(sb => {
        st.sb = sb; st.sceneId = fromId;
        renderStoryboardScenes(); renderStoryboardEditor();
        toast('已调整顺序');
    }).catch(e => toast('排序保存失败：' + e.message));
}
async function storyboardDeleteScene(sceneId){
    const st = CanvasPanelState.storyboard;
    const sid = sceneId || st.sceneId;
    if(!sid || !st.sb) return;
    if(!window.confirm('删除该场景？（其已生成的任务与产物不受影响）')) return;
    const scenes = sortedStoryboardScenes().filter(s => s.scene_id !== sid);
    try{
        const sb = await storyboardSavePayload(scenes, st.sb.name);
        st.sb = sb; st.sceneId = (sb.scenes[0] || {}).scene_id || null;
        renderStoryboardScenes(); renderStoryboardEditor();
        toast('已删除场景');
    }catch(e){ toast('删除场景失败：' + e.message); }
}
function storyboardReadCfg(){
    return {
        provider_id: document.getElementById('storyboardCfgProvider').value.trim() || 'comfly',
        model: document.getElementById('storyboardCfgModel').value.trim() || 'veo3-fast',
        duration: parseInt(document.getElementById('storyboardCfgDuration').value) || 5,
        aspect_ratio: document.getElementById('storyboardCfgAspect').value.trim() || '16:9'
    };
}
function storyboardSavePayload(scenes, name){
    const st = CanvasPanelState.storyboard;
    const body = {
        name: name || '',
        scenes: scenes.map((s, i) => ({
            ...
            transition: s.transition || '', audio: s.audio || ''
        })),
        video_config: storyboardReadCfg()
    };
    return canvasApi('/api/storyboards/' + encodeURIComponent(st.sb.storyboard_id), {method:'PUT', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body)});
}
async function storyboardSaveScene(){
    const st = CanvasPanelState.storyboard;
    if(!st.sb) return;
    const scene = (st.sb.scenes || []).find(s => s.scene_id === st.sceneId);
    if(!scene) return;
    scene.visual = document.getElementById('storyboardVisual').value.trim();
    scene.prompt = document.getElementById('storyboardPrompt').value.trim();
    scene.camera = document.getElementById('storyboardCamera').value.trim();
    scene.motion = document.getElementById('storyboardMotion').value.trim();
    scene.duration = parseInt(document.getElementById('storyboardDuration').value) || 0;
    scene.transition = document.getElementById('storyboardTransition').value;
    scene.audio = document.getElementById('storyboardAudio').value.trim();
    const scenes = sortedStoryboardScenes();
    try{
        const sb = await storyboardSavePayload(scenes, document.getElementById('storyboardName').value.trim());
        st.sb = sb;
        renderStoryboardScenes(); renderStoryboardEditor();
        toast('已保存');
    }catch(e){ toast('保存失败：' + e.message); }
}
async function storyboardRenderScene(){
    const st = CanvasPanelState.storyboard;
    const scene = (st.sb?.scenes || []).find(s => s.scene_id === st.sceneId);
    if(!scene){ toast('请先选择场景'); return; }
    if(!scene.prompt && !scene.visual){ toast('请先填写提示词或选择参考图'); return; }
    try{
        const r = await canvasApi(`/api/storyboards/${encodeURIComponent(st.sb.storyboard_id)}/render?scene_id=${encodeURIComponent(scene.scene_id)}`, {method:'POST'});
        toast('已创建任务 ' + String(r.task_id).slice(-12) + '，进入 Task Engine 队列');
        await refreshStoryboardDetail();
    }catch(e){ toast('渲染失败：' + e.message); }
}
async function storyboardRenderAll(){
    const st = CanvasPanelState.storyboard;
    if(!st.sb) return;
    const n = st.sb.scenes?.length || 0;
    if(!n){ toast('请先添加场景'); return; }
    if(!window.confirm(`渲染全部 ${n} 个场景？将按顺序为每个场景创建真实视频生成任务（已结束的场景会重新创建任务）。`)) return;
    try{
        const r = await canvasApi(`/api/storyboards/${encodeURIComponent(st.sb.storyboard_id)}/render-all`, {method:'POST'});
        toast(r.message || `已创建 ${r.launched} 个任务`);
        await refreshStoryboardDetail();
    }catch(e){ toast('渲染失败：' + e.message); }
}
async function storyboardDelete(){
    const st = CanvasPanelState.storyboard;
    if(!st.sb) return;
    if(!window.confirm(`删除分镜「${st.sb.name}」？（已生成的产物与任务历史不受影响）`)) return;
    try{
        await canvasApi('/api/storyboards/' + encodeURIComponent(st.sb.storyboard_id), {method:'DELETE'});
        toast('已删除');
        stopStoryboardPolling();
        st.sb = null; st.sceneId = null;
        await loadStoryboardsPanel();
        renderStoryboardScenes(); renderStoryboardEditor();
    }catch(e){ toast('删除失败：' + e.message); }
}
function startStoryboardPolling(){
    stopStoryboardPolling();
    CanvasPanelState.storyboard.timer = setInterval(refreshStoryboardDetail, 3000);
}
function stopStoryboardPolling(){
    if(CanvasPanelState.storyboard.timer){ clearInterval(CanvasPanelState.storyboard.timer); CanvasPanelState.storyboard.timer = null; }
}
async function refreshStoryboardDetail(){
    const st = CanvasPanelState.storyboard;
    if(!st.sb) return;
    try{
        const sb = await canvasApi('/api/storyboards/' + encodeURIComponent(st.sb.storyboard_id));
        const prevSceneId = st.sceneId;
        const wasRendering = (st.sb.scenes || []).some(s => s.status === 'running' || s.status === 'queued');
        st.sb = sb;
        if(prevSceneId && !sb.scenes.some(s => s.scene_id === prevSceneId)) st.sceneId = (sb.scenes[0] || {}).scene_id || null;
        renderStoryboardScenes(); renderStoryboardEditor();
        const nowRendering = sb.scenes.some(s => s.status === 'running' || s.status === 'queued');
        if(wasRendering && !nowRendering) toast('全部渲染任务已结束');
    }catch(e){ /* 轮询失败静默 */ }
}
/* 新建分镜 */
function storyboardCreateOpen(){
    canvasModalOpen('新建分镜', `
        <div class="canvas-field"><label>分镜名称</label><input id="storyboardCreateName" placeholder="例如：品牌宣传片分镜" maxlength="120"></div>
        <div class="canvas-drawer-toolbar" style="margin-top:14px;justify-content:flex-end">
            <button class="canvas-btn" onclick="canvasModalClose()">取消</button>
            <button class="canvas-btn primary" onclick="storyboardCreate()">创建分镜</button>
        </div>`);
}
async function storyboardCreate(){
    const name = (document.getElementById('storyboardCreateName').value || '').trim();
    if(!name){ toast('请填写分镜名称'); return; }
    try{
        const sb = await canvasApi('/api/storyboards', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({name, scenes: []})});
        canvasModalClose();
        toast(`已创建「${sb.name}」`);
        await loadStoryboardsPanel();
        await openStoryboardPanelItem(sb.storyboard_id);
    }catch(e){ toast('创建失败：' + e.message); }
}
/* 参考图选择 */
async function storyboardPickOpen(){
    const st = CanvasPanelState.storyboard;
    st.pickTab = 'outputs';
    canvasModalOpen('选择参考图 / 参考视频', `
        <div class="pick-tabs">
            <button class="pick-tab active" id="canvasPickTabOutputs" onclick="storyboardPickTab('outputs')">输出库</button>
            <button class="pick-tab" id="canvasPickTabAssets" onclick="storyboardPickTab('assets')">素材库</button>
        </div>
        <input class="pick-search" id="canvasPickSearch" placeholder="搜索名称 / URL…" oninput="storyboardPickRender()">
        <div class="pick-grid" id="canvasPickGrid"><div class="pick-empty">加载中…</div></div>`);
    await storyboardPickLoad();
}
async function storyboardPickTab(tab){
    const st = CanvasPanelState.storyboard;
    st.pickTab = tab;
    document.getElementById('canvasPickTabOutputs').classList.toggle('active', tab === 'outputs');
    document.getElementById('canvasPickTabAssets').classList.toggle('active', tab === 'assets');
    document.getElementById('canvasPickSearch').value = '';
    await storyboardPickLoad();
}
async function storyboardPickLoad(){
    const st = CanvasPanelState.storyboard;
    const grid = document.getElementById('canvasPickGrid');
    if(!grid) return;
    grid.innerHTML = '<div class="pick-empty">加载中…</div>';
    try{
        if(st.pickTab === 'outputs'){
            const d = await canvasApi('/api/outputs?kind=image&limit=200');
            st.pickItems = (d.items || []).map(i => ({url: i.url, name: i.name || i.url, type: '图'}));
        }else{
            const d = await canvasApi('/api/asset-library');
            const items = [];
        ...
        storyboardPickRender();
    }catch(e){
        grid.innerHTML = `<div class="pick-empty">加载失败：${escapeHtml(e.message)}</div>`;
    }
}
function storyboardPickRender(){
    const st = CanvasPanelState.storyboard;
    const q = (document.getElementById('canvasPickSearch').value || '').trim().toLowerCase();
    const grid = document.getElementById('canvasPickGrid');
    if(!grid) return;
    const items = st.pickItems.filter(i => !q || String(i.name).toLowerCase().includes(q) || String(i.url).toLowerCase().includes(q));
    grid.innerHTML = '';
    if(!items.length){ grid.innerHTML = '<div class="pick-empty">没有找到素材</div>'; return; }
    items.slice(0, 240).forEach(i => {
        const div = document.createElement('div');
        div.className = 'pick-item';
        div.innerHTML = `<img src="${escapeAttr(i.url)}" loading="lazy" onerror="this.style.display='none'"><div class="pname">${escapeHtml(i.name)}</div><span class="ptype">${i.type}</span>`;
        div.title = i.url;
        div.addEventListener('click', () => {
            document.getElementById('storyboardVisual').value = i.url;
            renderStoryboardVisualPreview(i.url);
            canvasModalClose();
            toast('已选择参考素材');
        });
...
/* V2.5: 画布内功能抽屉（输出/批量/分镜）导出 */
window.canvasModalClose = canvasModalClose;
window.canvasTaskCancel = canvasTaskCancel;
window.canvasTaskRetry = canvasTaskRetry;
window.closeCanvasPanel = closeCanvasPanel;
window.matrixAddDimRow = matrixAddDimRow;
window.matrixBackToList = matrixBackToList;
window.matrixCancel = matrixCancel;
window.matrixCreate = matrixCreate;
window.matrixDelete = matrixDelete;
window.matrixPreviewCount = matrixPreviewCount;
window.matrixRun = matrixRun;
window.openCanvasPanel = openCanvasPanel;
window.openCanvasTaskModal = openCanvasTaskModal;
window.openMatrixCreatePanel = openMatrixCreatePanel;
window.outputsAddToCanvas = outputsAddToCanvas;
window.outputsDoAddToCanvas = outputsDoAddToCanvas;
window.outputsDownload = outputsDownload;
window.outputsPreview = outputsPreview;
window.outputsSearchInput = outputsSearchInput;
window.outputsSetFav = outputsSetFav;
window.outputsSetKind = outputsSetKind;
window.outputsToggleFav = outputsToggleFav;
window.refreshMatricesPanel = refreshMatricesPanel;
window.refreshMatrixDetail = refreshMatrixDetail;
window.refreshOutputsPanel = refreshOutputsPanel;
window.refreshStoryboardsPanel = refreshStoryboardsPanel;
window.storyboardAddScene = storyboardAddScene;
window.storyboardClearVisual = storyboardClearVisual;
window.storyboardCreate = storyboardCreate;
window.storyboardCreateOpen = storyboardCreateOpen;
window.storyboardDelete = storyboardDelete;
window.storyboardDeleteScene = storyboardDeleteScene;
window.storyboardMoveScene = storyboardMoveScene;
window.storyboardPickOpen = storyboardPickOpen;
window.storyboardPickRender = storyboardPickRender;
window.storyboardPickTab = storyboardPickTab;
window.storyboardRenderAll = storyboardRenderAll;
window.storyboardRenderScene = storyboardRenderScene;
window.storyboardSaveScene = storyboardSaveScene;
window.storyboardSwitch = storyboardSwitch;
