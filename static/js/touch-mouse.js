...
(function(){
    ...
    /* —— shouldSkip 缓存机制 ——
       原实现每次触摸都 getComputedStyle + scrollHeight/clientHeight 遍历祖先，
       触发强制布局重排（forced reflow）。改为 WeakMap 缓存每个元素的滚动状态，
       仅在首次访问和 resize 后重新计算，大幅减少布局重排次数。 */
    const scrollCache = new WeakMap();
    let cacheStamp = Date.now();           // 缓存时间戳，resize 时更新

    window.addEventListener('resize', () => {
        cacheStamp = Date.now();            // 标记缓存失效，下次 shouldSkip 重新计算
    }, { passive: true });
